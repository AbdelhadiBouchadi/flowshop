...
export default function ProductMedia({ media }: ProductMediaProps) {
  ...
  return (
    <div className="h-fit basis-2/5 space-y-5 md:sticky md:top-[18vh] 2xl:top-24">
      <div className="aspect-square rounded-md bg-secondary shadow-md">
        {selectedImage?.url ? (
          ...
          <div className="flex size-full items-center rounded-md bg-secondary shadow-md">
            <video controls className="size-full rounded-md">
              <source
...
function MediaPreview({ mediaItem, isSelected, onSelect }: MediaPreviewProps) {
  ...
  return (
    <div
      className={cn(
        "relative cursor-pointer rounded-md bg-secondary shadow-md",
